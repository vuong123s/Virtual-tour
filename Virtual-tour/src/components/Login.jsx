...
const Login = ({ isPopupActive, onClose }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8000/v1/auth/login', { email, password });
  ...
  const handleRegister = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post('http://localhost:8000/v1/auth/register', formData);
      const { accessToken, ...user } = response.data;
